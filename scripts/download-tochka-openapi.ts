...
const TARGET_TAG = "Работа с платёжными ссылками";

const BASE_URL = envs.TOCHKA_OPENAPI_URL?.replace(/\/+$/, "");

if (!BASE_URL) {
  console.error("❌ Ошибка: envs.TOCHKA_BASE_API_URL не задан");
  process.exit(1);
}

const DOWNLOAD_URL = `${BASE_URL}/doc/openapi/swagger.json`;
...
async function main() {
  ...
  try {
    ...
    console.log(`🧹 Фильтрую по тегу "${TARGET_TAG}" + очищаю компоненты...`);

    const filterResult = await openapiFilter(spec, {
      filterSet: {
        inverseTags: [TARGET_TAG],
        unusedComponents: [
          "schemas",
          "parameters",
          "requestBodies",
